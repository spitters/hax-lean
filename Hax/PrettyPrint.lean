...
-- The unverified fold control-flow transforms (transformWhileBreak /
-- transformWhileContinue / transformWhileFoldBody / patchCfBreakUnit /
...
-- `qualifyProjections` and its helpers (`collectProjectionsOnVar`,
-- `inferStructType`, `inferExprStructType`) deleted 2026-05-18 — the
-- typed `Hax.tQualifyProjections` (in Hax/TPhase/QualifyProjections.lean)
-- subsumes them. The typed phase runs on TExpr before erase using direct
-- `arg.ty` annotations instead of these post-erase heuristics.
--
-- `rewriteAppName` moved to `Hax/Phase/RewriteAppName.lean`.
-- Imported above; re-exposed in the `Hax` namespace.

-- `rewriteNewToStructCtor` and `detectStructArrayInit` deleted 2026-05-18 —
-- the typed `Hax.tRewriteNewToStructCtor` (in
-- Hax/TPhase/RewriteNewToStructCtor.lean) subsumes them. The typed phase
-- reads outer `.mk _ ty` annotations directly instead of running
-- arity-and-arg-name disambiguation heuristics over the erased form.

-- `rewriteStructFromElem`, `fixProjectionPaths`, and `detectStructDeps`
-- deleted 2026-05-18 — the typed `Hax.tRewriteStructFromElem` (in
-- Hax/TPhase/RewriteStructFromElem.lean) and `Hax.tFixProjectionPaths`
-- (in Hax/TPhase/FixProjectionPaths.lean) subsume them. The unlock was
-- adding `.ann` unwrapping in the typed phase — the hax JSON adapter
-- wraps `vec![T::ZERO; n]`-style macro expansions in `.ann` type
-- ascription nodes; without unwrapping, the typed phase silently
-- missed those calls and we depended on the untyped heuristic
-- fallback. `detectStructDeps` had zero call sites (already dead).
...
def toLeanCertifiedFile (defs : List (String × ImpExpr))
  ...
  -- Type-dependent rewriters (qualifyProjections, rewriteNewToStructCtor,
  -- rewriteStructFromElem, fixProjectionPaths) were removed 2026-05-18.
  -- This function is `@[deprecated]` and exists only for `--emit-certified`
  -- without `--hax` (no production consumer). The output for that legacy
  -- path is now slightly less polished than it was, but the function still
  -- runs to keep the MainT call site building; the warning emitted from
  -- MainT.lean tells users to switch to `--emit-certified --hax`.
  ...
  -- `rewriteStructFromElem` and `fixProjectionPaths` were removed
  -- 2026-05-18 along with the three other type-dependent rewriters;
  -- see comment above near `applyTypedPasses` for the rationale.
