...
/-- Extended builtins with bitwise operations.
    Falls back to `SSProve.Hax.defaultBuiltins` for arithmetic/comparison. -/
def bitwiseBuiltins : SSProve.Hax.Builtins
  ...
  | "bitxor", [.int a, .int b] => some (.int (↑(a.toNat ^^^ b.toNat)))
  | "cast",   [v] => some v
  | f, args => SSProve.Hax.defaultBuiltins f args
...
theorem widthArithOps_noControlFlow :
    SSProve.Hax.Builtins.NoControlFlow SSProve.Hax.widthArithOps := by
  intro f args v h isBreak w heq; subst heq; revert h
  simp only [SSProve.Hax.widthArithOps, SSProve.Hax.wrapUint]
  split <;> intro h
  all_goals (first | cases h | (split at h <;> cases h))

theorem widthBitwiseOps_noControlFlow :
    SSProve.Hax.Builtins.NoControlFlow SSProve.Hax.widthBitwiseOps := by
  intro f args v h isBreak w heq; subst heq; revert h
  simp only [SSProve.Hax.widthBitwiseOps, SSProve.Hax.wrapUint]
  split <;> (intro h; cases h)

theorem widthCmpOps_noControlFlow :
    SSProve.Hax.Builtins.NoControlFlow SSProve.Hax.widthCmpOps := by
  intro f args v h isBreak w heq; subst heq; revert h
  simp only [SSProve.Hax.widthCmpOps]
  split <;> (intro h; cases h)

theorem widthCastOps_noControlFlow :
    SSProve.Hax.Builtins.NoControlFlow SSProve.Hax.widthCastOps := by
  intro f args v h isBreak w heq; subst heq; revert h
  simp only [SSProve.Hax.widthCastOps]
  split <;> (intro h; cases h)

theorem widthArrayOps_noControlFlow :
    SSProve.Hax.Builtins.NoControlFlow SSProve.Hax.widthArrayOps := by
  intro f args v h isBreak w heq; subst heq; revert h
  simp only [SSProve.Hax.widthArrayOps]
  split
...
theorem widthAwareBuiltins_noControlFlow :
    SSProve.Hax.Builtins.NoControlFlow SSProve.Hax.widthAwareBuiltins := by
  intro f args v h isBreak w heq; subst heq
  delta SSProve.Hax.widthAwareBuiltins at h
  cases hwo : SSProve.Hax.widthOps f args with
  | some v' =>
    rw [hwo] at h; cases h
    exact widthOps_noControlFlow f args _ hwo isBreak w rfl
  | none =>
    rw [hwo] at h
    exact SSProve.Hax.Builtins.defaultBuiltins_noControlFlow f args _ h isBreak w rfl

/-! ## 8. Panic/Unwrap Operations NoControlFlow -/

theorem panicOps_noControlFlow :
    SSProve.Hax.Builtins.NoControlFlow SSProve.Hax.panicOps := by
  intro f args v h isBreak w heq; subst heq; revert h
  simp only [SSProve.Hax.panicOps]
  split <;> (intro h; cases h)

/-- `fullBuiltins` never produces ControlFlow values. -/
theorem fullBuiltins_noControlFlow :
    SSProve.Hax.Builtins.NoControlFlow SSProve.Hax.fullBuiltins := by
  intro f args v h isBreak w heq; subst heq
  delta SSProve.Hax.fullBuiltins at h
  cases hwo : SSProve.Hax.widthOps f args with
  | some v' =>
  ...
  | none =>
    simp [hwo] at h
    cases hpo : SSProve.Hax.panicOps f args with
    | some v' =>
