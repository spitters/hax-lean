...
/-- Generate a Lean 4 definition from a TExpr, using types from the TExpr tree
    for parameter annotations. Falls back to `toLeanDef` for the body rendering. -/
def toLeanDefTyped (name : String) (te : TExpr)
    (structLookup : String → Option String := fun _ => none)
    (structMeta : StructMeta := [])
    (allFnTypes : List (String × HaxAdapter.FnTypeInfo) := []) : String :=
  let (tparams, tbody) := extractTParams te
  let body := tbody.erase
...
/-- Generate a complete certified Lean 4 file from typed TExpr definitions.
    This is the typed counterpart of `toLeanCertifiedFile`, using types from
    the TExpr tree instead of heuristic type recovery. -/
def toLeanCertifiedFileTyped (tdefs : List (String × TExpr))
    (moduleName : String := "Generated")
    (structMeta : StructMeta := [])
    (fnTypes : List (String × HaxAdapter.FnTypeInfo) := []) : String :=
  -- Deduplicate
  let tdefs := tdefs.foldl (fun (acc : List (String × TExpr)) (n, te) =>
    if acc.any (·.1 == n) then acc else acc ++ [(n, te)]) []
  -- Erase for untyped passes that we still need
  let defs := tdefs.map fun (n, te) => (n, te.erase)
  -- Apply typed passes (struct projection disambiguation, etc.)
  let (defs, fnTypes) := applyTypedPasses defs structMeta fnTypes []
  -- Re-associate typed info: use erased defs but typed metadata from tdefs
  let tdefs := defs.map fun (n, e) =>
    -- Find the original TExpr for type info
    match tdefs.find? (·.1 == n) with
    | some (_, te) => (n, te)
    | none => (n, TExpr.ofImpExpr e)
  let structIsPassthrough := computeStructPassthrough structMeta defs
  let structLookup := mkStructLookup structMeta structIsPassthrough
  ...
  -- Generate preamble using typed information
  let (preamble, projConflicts) := generatePreambleTyped tdefs moduleName structMeta fnTypes
  ...
  let body := "\n".intercalate (defs.map fun (n, e) =>
    ...
    -- Use typed definition when we have TExpr info, otherwise fall back to untyped
    let surfaceDef := match tdefs.find? (·.1 == n) with
      | some (_, te) => toLeanDefTyped n te (structLookup := structLookup) (structMeta := structMeta) (allFnTypes := fnTypes)
