...
def toLeanCertifiedFileTyped (rawTdefs : List (String × TExpr))
  ...
  let newtypeBlock : String :=
    if newtypeRenamed.isEmpty then ""
      ...
      "/-- Newtype tuple-struct aliases: transparent type equalities\n    with definitional `.0` unwraps. Inner types may themselves be\n    axiomatized (see the axiom block above). -/\n"
        ++ "\n".intercalate lines ++ "\n\n"
  -- `--emit-certified` output is CatCrypt-bound: it imports
  -- `CatCrypt.Crypto.Jasmin.LowCT` for the sibling `_lowct` defs, so the
  -- runtime imports are CatCrypt-prefixed to match. Emitting bare `Hax.*` here
  -- would name modules that resolve against the upstream `Hax` Rust repo when
  -- it is a Lake dependency, whose `proof-libs/lean/Hax/` has no `Runtime.lean`.
  -- The untyped path's standalone preamble (`PrettyPrint.lean`) is unaffected.
  let header := s!"/-\n  Auto-generated by haxpipeT --emit-certified (typed extraction pipeline)\n  Surface code + ImpExpr literals for agreement proofs.\n-/\nimport CatCrypt.Hax.Runtime\nimport CatCrypt.Hax.AST\nimport CatCrypt.Hax.Semantics\nimport CatCrypt.Crypto.Jasmin.LowCT\nimport CatCrypt.Crypto.Hax.HaxToLowCT\nimport CatCrypt.Crypto.SecureCompilation.VIR.KernelDelab\n\nset_option linter.unusedVariables false\nset_option maxRecDepth 2048\nset_option maxHeartbeats 6400000\n\nnamespace {moduleName}\n\nopen CatCrypt.Hax\nopen CatCrypt.Crypto.Hax\nopen CatCrypt.Crypto.Jasmin.LowCT\n\n-- All emitted functions are `noncomputable`: extracted bodies may\n-- depend on Runtime axioms (sha256, bridgeCast, ...) which the Lean\n-- code generator rejects. Verification doesn't require execution.\nnoncomputable section\n\n{axiomsBlock}{inductiveBlock}{newtypeBlock}{preamble}\n{typeAliasBlock}mutual\n\n"
  ...
  let impExprs := "\n".intercalate (defs.map fun (n, e) =>
    ...
    s!"{impExprDef}")
  let lowCTs := "\n".intercalate (defs.map fun (n, e) =>
    let lowCTDef := toLeanLowCTDef n e
    let lowCTDef := if needsPartial then lowCTDef.replace "def " "partial def " else lowCTDef
    s!"{lowCTDef}")
