  ...
  let newtypeBlock : String :=
    if newtypeRenamed.isEmpty then ""
      ...
      "/-- Newtype tuple-struct aliases: transparent type equalities\n    with definitional `.0` unwraps. Inner types may themselves be\n    axiomatized (see the axiom block above). -/\n"
        ++ "\n".intercalate lines ++ "\n\n"
  -- `--emit-certified` output is CatCrypt-bound, so the runtime imports are
  -- CatCrypt-prefixed. Emitting bare `Hax.*` here would name modules that
  -- resolve against the upstream `Hax` Rust repo when it is a Lake dependency,
  -- whose `proof-libs/lean/Hax/` has no `Runtime.lean`. The untyped path's
  -- standalone preamble (`PrettyPrint.lean`) is unaffected.
